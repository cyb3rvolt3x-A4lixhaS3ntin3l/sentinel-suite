...
| **Status** | **Phase C slice7 shipped** — `xss_dom` sink-proof pack v0 (source→sink marker evidence; reflected/stored stubs) + `graphql` / `race_toctou` / `business_logic` / `bola_idor_bfla` / `ato_oauth_oidc` intact; Phase B Eye intact. Engine hashes **HOLD** (allowlist empty). |
...
Hunt packs need Role A at `roles/a.json` (`cookies|headers|bearer`) unless `needs_roles=0` (`graphql` / `xss_dom` — Role A optional); `bola_idor_bfla` also requires Role B at `roles/b.json`. `business_logic` / `graphql` / `xss_dom` findings stay `needs_human` until `sentinel hunt confirm-finding`.
