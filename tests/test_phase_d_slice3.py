...
def test_settings_payload_read(home):
    ...
    assert out["phase"] in ("D3", "D4", "E0", "E1", "E2", "E3", "G0")
...
def test_api_coach_and_settings_routes(ui_server_skip):
    ...
    assert code == 200
    assert health["phase"] in ("D3", "D4", "E0", "E1", "E2", "E3")
...
def test_coach_settings_ro_without_auth(home):
    ...
    try:
        ...
        code, settings = _http_json(f"{base}/api/settings")
        assert code == 200
        assert settings["phase"] in ("D3", "D4", "E0", "E1", "E2", "E3", "G0")
    finally:
        httpd.shutdown()
        httpd.server_close()
...
def test_spa_mentions_d3_screens():
    ...
    assert "Phase D3" in html or "Phase D4" in html or "E2 labs" in html or "Phase G0" in html
