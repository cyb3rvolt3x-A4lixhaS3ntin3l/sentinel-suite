"""Hard request caps for jwt_session pack live-mock paths.

Fixture-only runs do not consume the budget. Any live/opener mock path must
respect HARD_MAX_REQUESTS (non-overridable). No live IdP hammering / token
exfil / session-hijack runbooks against real IdPs.
"""
...
COACH_CAPS = (
    "jwt_session pack hard request cap: max requests≤{requests} when any live "
    "mock is used. Fixture-only runs are uncapped by this gate. "
    "Authorized / lab only — no live IdP hammering; no token exfil."
)


class JwtSessionCapExceededError(Exception):
    ...
    *,
    ...
    if max_requests is None:
        ...
        try:
            ...
            raise JwtSessionCapExceededError(
                f"jwt_session invalid max_requests={max_requests!r}. "
                + COACH_CAPS.format(requests=HARD_MAX_REQUESTS),
                exit_code=2,
            ) from exc
    if r < 1:
        raise JwtSessionCapExceededError(
            f"jwt_session max_requests must be ≥1 (got {r}). "
            + COACH_CAPS.format(requests=HARD_MAX_REQUESTS),
            exit_code=2,
        )
    ...
    return JwtSessionCaps(max_requests=r, i_understand_lab=bool(i_understand_lab))
