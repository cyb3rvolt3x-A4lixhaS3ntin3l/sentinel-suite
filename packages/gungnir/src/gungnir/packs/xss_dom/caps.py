"""Hard request caps for xss_dom pack live-mock paths.

Fixture-only runs do not consume the budget. Any live/opener mock path must
respect HARD_MAX_REQUESTS (non-overridable). No dalfox binary / --tools.
"""
...
COACH_CAPS = (
    "xss_dom pack hard request cap: max requests≤{requests} when any live "
    "mock is used. Fixture-only runs are uncapped by this gate. "
    "Authorized / lab only — no live mass XSS scanning; no dalfox-all."
)


class XssDomCapExceededError(Exception):
    ...
    *,
    ...
    if max_requests is None:
        ...
        try:
            ...
            raise XssDomCapExceededError(
                f"xss_dom invalid max_requests={max_requests!r}. "
                + COACH_CAPS.format(requests=HARD_MAX_REQUESTS),
                exit_code=2,
            ) from exc
    if r < 1:
        raise XssDomCapExceededError(
            f"xss_dom max_requests must be ≥1 (got {r}). "
            + COACH_CAPS.format(requests=HARD_MAX_REQUESTS),
            exit_code=2,
        )
    ...
    return XssDomCaps(max_requests=r, i_understand_lab=bool(i_understand_lab))
