"""Hard request caps for cache_host pack live-mock paths.

Fixture-only runs do not consume the budget. Any live/opener mock path must
respect HARD_MAX_REQUESTS (non-overridable). No mass Host-header spray /
production CDN poison tooling.
"""
...
COACH_CAPS = (
    "cache_host pack hard request cap: max requests≤{requests} when any live "
    "mock is used. Fixture-only runs are uncapped by this gate. "
    "Authorized / lab only — no mass Host-header spray; no production CDN poison."
)


class CacheHostCapExceededError(Exception):
    ...
    *,
    ...
    if max_requests is None:
        ...
        try:
            ...
            raise CacheHostCapExceededError(
                f"cache_host invalid max_requests={max_requests!r}. "
                + COACH_CAPS.format(requests=HARD_MAX_REQUESTS),
                exit_code=2,
            ) from exc
    if r < 1:
        raise CacheHostCapExceededError(
            f"cache_host max_requests must be ≥1 (got {r}). "
            + COACH_CAPS.format(requests=HARD_MAX_REQUESTS),
            exit_code=2,
        )
    ...
    return CacheHostCaps(max_requests=r, i_understand_lab=bool(i_understand_lab))
