"""Hard request caps for open_redirect pack live-mock paths.

Fixture-only runs do not consume the budget. Any live/opener mock path must
respect HARD_MAX_REQUESTS (non-overridable). No blind param spray / redirect farms.
"""
...
COACH_CAPS = (
    "open_redirect pack hard request cap: max requests≤{requests} when any live "
    "mock is used. Fixture-only runs are uncapped by this gate. "
    "Authorized / lab only — no live mass redirect spray; no param farms."
)


class OpenRedirectCapExceededError(Exception):
    ...
    *,
    ...
    if max_requests is None:
        ...
        try:
            ...
            raise OpenRedirectCapExceededError(
                f"open_redirect invalid max_requests={max_requests!r}. "
                + COACH_CAPS.format(requests=HARD_MAX_REQUESTS),
                exit_code=2,
            ) from exc
    if r < 1:
        raise OpenRedirectCapExceededError(
            f"open_redirect max_requests must be ≥1 (got {r}). "
            + COACH_CAPS.format(requests=HARD_MAX_REQUESTS),
            exit_code=2,
        )
    ...
    return OpenRedirectCaps(max_requests=r, i_understand_lab=bool(i_understand_lab))
