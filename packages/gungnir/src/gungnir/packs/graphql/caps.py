"""Hard request caps for graphql pack live-mock paths.

Fixture-only runs do not consume the budget. Any live/opener mock path must
respect HARD_MAX_REQUESTS (non-overridable).
"""
...
COACH_CAPS = (
    "graphql pack hard request cap: max requests≤{requests} when any live "
    "mock is used. Fixture-only runs are uncapped by this gate. "
    "Authorized / lab only — no third-party GraphQL hammering."
)


class GraphqlCapExceededError(Exception):
...
@dataclass
class RequestBudget:
    """Thread-safe-ish sequential budget for live mock requests."""
    ...
    *,
    ...
    if max_requests is None:
        ...
        try:
            ...
            raise GraphqlCapExceededError(
                f"graphql invalid max_requests={max_requests!r}. "
                + COACH_CAPS.format(requests=HARD_MAX_REQUESTS),
                exit_code=2,
            ) from exc
    if r < 1:
        raise GraphqlCapExceededError(
            f"graphql max_requests must be ≥1 (got {r}). "
            + COACH_CAPS.format(requests=HARD_MAX_REQUESTS),
            exit_code=2,
        )
    ...
    return GraphqlCaps(max_requests=r, i_understand_lab=bool(i_understand_lab))
